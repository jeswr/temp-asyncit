...
let READABLE_QUEUE: Set<AsyncIterator<any>> | null = null;
let DATA_EMIT_QUEUE: Set<AsyncIterator<any>> | null = null;
let ITEM_GENERATION_QUEUE: Set<AsyncIterator<any>> | null = null;
...
export function iteratorTask() {
  ...
  const _ITEM_GENERATION_QUEUE = ITEM_GENERATION_QUEUE;
  const _DATA_EMIT_QUEUE = DATA_EMIT_QUEUE;
  const _READABLE_QUEUE = READABLE_QUEUE;
...
// TODO: Test the performance (memory & time) of this method vs. the flag method
// I suspect this may be better in the cases where only a small number of iterators are needing to schedule events

// Actually make a note that we probably *don't* want this method as we want to retain the ordering of ques
export function queueDataEmission(iterator: any) {
  if (DATA_EMIT_QUEUE === null) {
    DATA_EMIT_QUEUE = new Set();
    queueIteratorTask();
  }
  DATA_EMIT_QUEUE.add(iterator);
}
